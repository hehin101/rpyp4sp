...
def run_test_case(ctx, value, config):
    ...
    Returns:
        tuple: (coverage_hash, crashed, timed_out, error_msg)
    """
...
def load_seeds(seed_files, ctx):
    ...
    for seed_file in seed_files:
        try:
            ...
            coverage_hash, crashed, timed_out, error_msg = run_test_case(
                ctx, seed_value, None
            )
            
            if crashed:
                print("  Seed crashed: %s" % error_msg)
            elif timed_out:
            ...
            seeds.append((seed_value, coverage_hash, error_msg if crashed else None))
        ...
        for iteration in range(config.max_iterations):
            ...
            for mutation_attempt in range(config.mutation_budget):
                try:
                    ...
                    coverage_hash, crashed, timed_out, error_msg = run_test_case(
                        ctx, mutated_value, config
                    )
                    
                    if crashed:
                        stats.crashes_found += 1
                        # Only print if this is a new error message
                        if stats.is_new_error(error_msg):
                            print("NEW CRASH at iteration %d: %s" % (iteration, error_msg))
                        # Save crashing input
                        ...
                        if crash_filename and stats.seen_error_messages[error_msg] == 1:
